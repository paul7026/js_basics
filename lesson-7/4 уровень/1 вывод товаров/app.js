...
let productBlock = document.querySelector(".products");
let butons = document.querySelectorAll("button");
butons.forEach(function (element) {
...
function clickHandler(event) {
  ...
  showCategory(event.target.attributes["data-type"].value);
}
...
function showCategory(category) {
  let categoryType = products[category];
  categoryType.forEach(function (product) {
    productBlock.insertAdjacentHTML("afterbegin", getProductMarkup(product));
  });
}
